...
let d = new Date();
let newDate = `${d.getMonth() + 1}.${d.getDate()}.${d.getFullYear()}`;  //  Date format
 ...
 */
const getData = async (zipCode, countryCode) => {
...
const update = async () => {
    ...
    try{
        ...
        document.querySelector('#state').innerHTML = `Country: ${updatingData.country}`;
        document.querySelector('#name').innerHTML = `Location: ${updatingData.name}`;
        document.querySelector('#date').innerHTML = `Date: ${updatingData.date}`;
        document.querySelector('#temp').innerHTML = `Temperature: ${updatingData.temp}°C`;
        document.querySelector('#content').innerHTML = `Feeling: ${updatingData.feelings}`;
        return updatingData;
    }catch(error){
...
button.addEventListener('click', getWeatherInfo = () => {
    // Preparing zipCode, feelings and countryCode
    ...
    if(!zipCode){   //  Check if the user wrote zipCode
        ...
        getData(zipCode, countryCode).then((data) => postData("/sendData", {
